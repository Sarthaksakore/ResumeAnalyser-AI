...
# Connect to database
connection = pymysql.connect(host='localhost', user='root', password='Sarthak@3117', db='cv')
...
st.set_page_config(
   ...
   page_icon='./Logo/logo5.PNG',
)

def run():
    img = Image.open('./Logo/logo5.PNG')
    ...
    link = '[©Developed by O-10](https://www.linkedin.com/)'
    ...
    if choice == 'Admin':
        ...
        if st.button('Login'):
            if ad_user == 'O10' and ad_password == '12345':
                st.success("Welcome!")
                ...
                # Decode binary columns to string if they exist
                if 'Predicted Field' in df.columns:
                    df['Predicted Field'] = df['Predicted Field'].apply(lambda x: x.decode('utf-8') if isinstance(x, bytes) else x)
                ...
                # Decode binary columns for plotting if they exist
                if 'Predicted Field' in plot_data.columns:
                    plot_data['Predicted Field'] = plot_data['Predicted Field'].apply(lambda x: x.decode('utf-8') if isinstance(x, bytes) else x)
                if 'User Level' in plot_data.columns:
                    plot_data['User Level'] = plot_data['User Level'].apply(lambda x: x.decode('utf-8') if isinstance(x, bytes) else x)

                with st.container():
                    # Plot for Predicted Field Recommendation
                    if 'Predicted Field' in plot_data.columns:
                        predicted_field_counts = plot_data['Predicted Field'].value_counts().reset_index()
                        predicted_field_counts.columns = ['Predicted_Field', 'count']
                        st.subheader("**Pie-Chart for Predicted Field Recommendation**")
                        fig1 = px.pie(predicted_field_counts, values='count', names='Predicted_Field', title='Predicted Field according to the Skills')
                        st.plotly_chart(fig1)

                    # Plot for User's Experienced Level
                    if 'User Level' in plot_data.columns:
                        user_level_counts = plot_data['User Level'].value_counts().reset_index()
                        user_level_counts.columns = ['User_level', 'count']
                        st.subheader("**Pie-Chart for User's Experienced Level**")
                        fig2 = px.pie(user_level_counts, values='count', names='User_level', title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
